...
var draw_map_key_categorical = function() {
    ...
    var axis_scale = d3.scaleOrdinal().domain(scale.domain()).rangeBands([scale.domain().length *20, 0])

    var svg = d3.select(VMT.mapholder.map.getPanes().overlayPane).append("svg")
        .attr('id', 'map_key')
        .attr("class", "leaflet-zoom-hide")
        .style("width", VMT.mapholder.map.getSize().x + 'px')
        .style("height", VMT.mapholder.map.getSize().y + 'px')
...
var draw_map_key_continuous = function() {
    ...
    var map_colour_scale = VMT.column_descriptions_data["demand_for_prison"]["colour_scale"];

    var num_cats = map_colour_scale.domain().length

    var axis_scale = d3.scaleLinear().domain(map_colour_scale.domain()).range(d3.range(key_height, -0.001, -key_height / (num_cats - 1)))
    ...
    var yAxis = d3.axisLeft()
        ...
        .ticks(10, ",0.2s")
        .tickSize(-10, 0)
        .tickFormat(VMT.column_descriptions_data["demand_for_prison"]["format"])
    ...
    svg.append("g")
        ...
        .append("text")
        .text(function(d) {
            return VMT.column_descriptions_data["demand_for_prison"]["long_name"]
        })
